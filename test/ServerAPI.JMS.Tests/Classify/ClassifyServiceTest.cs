using Xunit;
using ServerAPI.Services;
using ServerAPI.ViewModels;
using GenerateTables.Models;
using System;
using System.Linq;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Configuration;
using ServerAPI.Tests;
using ServerAPI.JMS.Tests.Helper;
using Moq;
using Microsoft.Extensions.Caching.Memory;
using Microsoft.AspNetCore.Identity;
using JwtDb.Models;

// ReSharper disable once CheckNamespace
namespace ServerAPI.JMS.Tests
{
    [Collection("Database collection")]
    public class ClassifyServiceTest
    {
        private readonly ClassifyService _classifyService;
        private readonly DbInitialize _fixture;
        private readonly IConfigurationRoot _configuration = MockHelpers.GetIConfigurationRoot();
        private readonly Mock<IMemoryCache> _memory = new Mock<IMemoryCache>();
        public ClassifyServiceTest(DbInitialize fixture)
        {
            _fixture = fixture;
            PersonService personService = new PersonService(_fixture.Db);
            AppletsSavedService appletsSavedService = new AppletsSavedService(fixture.Db, _configuration);
            InterfaceEngineService interfaceEngineService = new InterfaceEngineService(fixture.Db, _configuration,
                _memory.Object);
            Mock<UserManager<AppUser>> userMgr = MockHelpers.MockUserManager<AppUser>();
            HttpContextAccessor httpContext = new HttpContextAccessor { HttpContext = fixture.Context.HttpContext };
            PhotosService photosService = new PhotosService(fixture.Db, _configuration);
            CommonService commonService = new CommonService(_fixture.Db, _configuration,
                httpContext, personService, appletsSavedService, interfaceEngineService);
            FormsService formsService = new FormsService(_fixture.Db, httpContext, commonService, personService,
                interfaceEngineService);
            _classifyService = new ClassifyService(fixture.Db, commonService, formsService, httpContext, personService,
                interfaceEngineService);
        }

        [Fact]
        public void ClassifyService_InmateClassificationSummary()
        {
            //Act
            ClassificationVm classification = _classifyService.GetInmateClassificationSummary(100);
            //Assert
            foreach (var inmate in classification.LstInmateClassification)
            {
                if (inmate.InmateClassificationId == 5)
                {
                    Assert.Equal("INITIAL", inmate.ClassificationType);
                    Assert.Equal("MINIMUM", inmate.ClassificationReason);
                }
                else if (inmate.InmateClassificationId == 20)
                {
                    Assert.Equal("REVIEW", inmate.ClassificationType);
                    Assert.Equal("SANGEETHA", inmate.ReviewOfficer.PersonFirstName);
                    Assert.Equal("INMATE-DETAILS", inmate.Note);
                }
            }
        }

        [Fact]
        public void ClassifyService_GetInmateClassificationDetails()
        {
            InmateDetail inmateDetail = _classifyService.GetInmateClassificationDetails(7, 103);
            //InmateClassification
            Assert.Equal("PENDING FILE", inmateDetail.InmateClassification.ClassificationNarration);
            //Inmatedetail
            Assert.Equal("CHS000", inmateDetail.InmateNumber);
            //Person
            Assert.Equal("SURUTHI", inmateDetail.Person.PersonMiddleName);
        }

        [Fact]
        public async Task ClassifyService_InsertInmateClassificationEntry()
        {
            //Arrange
            InmateClassificationVm inmateClassifications = new InmateClassificationVm
            {
                ClassificationOfficerId = 13,
                DateAssigned = DateTime.Now,
                DateUnAssigned = DateTime.Now,
                ClassificationReason = "HISTORY",
                ClassificationNarration = "HISTORY RECORDS MAINTAIN",
                InmateId = 101
            };
            InmateDetail inmateDetail = new InmateDetail
            {
                InmateNumber = "INUM120",
                InmateCurrentTrack = "MADURAI",
                InmateActive = true,
                InmateClassification = inmateClassifications
            };
            InmateClassification inmateClassification =
                 _fixture.Db.InmateClassification.SingleOrDefault(i => i.InmateClassificationReason == " HISTORY");

            Assert.Null(inmateClassification);

            //Act
            await _classifyService.InsertInmateClassificationEntry(inmateDetail);

            //Assert
            inmateClassification = _fixture.Db.InmateClassification.Single(i => i.InmateClassificationReason == "HISTORY");
            Assert.Equal("HISTORY", inmateClassification.InmateClassificationReason);
            Assert.Equal(101, inmateClassification.InmateId);
        }

        [Fact]
        public async Task ClassifyService_UpdateClassification()
        {
            //Arrange
            InmateClassificationVm inmateClassifications = new InmateClassificationVm
            {
                InmateClassificationId = 6,
                InmateId = 101,
                DateAssigned = DateTime.Now.AddDays(-2),
                ClassificationReason = "MINIMUM",
                ClassificationNarration = "CASE HOLD ON",
                ClassifyFormData = new FormSaveData
                {
                    InmateClassificationId = 11,
                    InmateId = 135,
                    FormRecordId = 7,
                    TemplateId = 11,
                    Payload =
                        @"{'person_name':'PRAKASH','person_dob':'09/27/1994','currentpersonnellast':'ANANTH','currentpersonnelnumber':'87457215412','stampdate':'10/11/2017','stamptime':'15.15.00','person_sex':'MALE','address_state':'TN'}",
                    SignValues = new Signature
                    {
                        Signature1 = null,
                        Signature2 = "data:image/png;base64,",
                        Signature3 = null
                    }
                },
                FormTemplate = new Form
                {
                    FormTemplateId = 5
                },
                ClassificationOfficer = new PersonnelVm
                {
                    PersonId = 55
                }
            };
            InmateClassification inmateClassification =
                _fixture.Db.InmateClassification.Single(i => i.InmateClassificationId == 6);

            Assert.Equal("MAXIMUM", inmateClassification.InmateClassificationReason);
            Assert.Equal("CASE NOT COMPLETED", inmateClassification.InmateOverrideNarrative);

            WebServiceEventTypeHistory webServiceEventTypeHistory =
                _fixture.Db.WebServiceEventTypeHistory.SingleOrDefault(w => w.WebServiceEventParameter1 == "55");

            WebServiceEventQueue webServiceEventQueue =
                _fixture.Db.WebServiceEventQueue.SingleOrDefault(w => w.WebServiceEventParameter1 == "55");
            Assert.Null(webServiceEventTypeHistory);
            Assert.Null(webServiceEventQueue);

            //Act
            await _classifyService.UpdateClassification(inmateClassifications);
            //Assert
            inmateClassification = _fixture.Db.InmateClassification.Single(i => i.InmateClassificationId == 6);

            Assert.Equal("MINIMUM", inmateClassification.InmateClassificationReason);
            Assert.Equal("CASE HOLD ON", inmateClassification.InmateOverrideNarrative);
            //EventHandle  Web Service values
            webServiceEventTypeHistory = _fixture.Db.WebServiceEventTypeHistory
                .Single(w => w.WebServiceEventParameter1 == "55");
            Assert.Equal("6", webServiceEventTypeHistory.WebServiceEventParameter2);

            webServiceEventQueue = _fixture.Db.WebServiceEventQueue
                .Single(w => w.WebServiceEventParameter1 == "55");

            Assert.NotNull(webServiceEventQueue);
        }

        [Fact]
        public async Task ClassifyService_InsertReviewEntry()
        {
            //Arrange
            InmateDetail inmateDetail = new InmateDetail
            {
                InmateId = 105,
                InmateClassification = new InmateClassificationVm
                {
                    ClassificationNarration = "INTERVIEW PROCESS"
                }
            };
            InmateClassificationNarrative inmateClassificationNarrative =
                _fixture.Db.InmateClassificationNarrative.SingleOrDefault(i => i.InmateId == 105);

            Assert.Null(inmateClassificationNarrative);
            //Act
            await _classifyService.InsertReviewEntry(inmateDetail);
            //Assert
            inmateClassificationNarrative = _fixture.Db.InmateClassificationNarrative.Single(i => i.InmateId == 105);
            Assert.Equal("INTERVIEW PROCESS", inmateClassificationNarrative.Narrative);
        }

        [Fact]
        public async Task ClassifyService_UpdateClassificationReview()
        {
            //Arrange
            InmateClassificationVm inmateClassifications = new InmateClassificationVm
            {
                ClassificationNarration = "FILE CONVERSION",
                ClassificationNarrativeId = 23
            };
            InmateClassificationNarrative inmateClassificationNarrative =
                _fixture.Db.InmateClassificationNarrative.Single(i => i.InmateClassificationNarrativeId == 23);

            Assert.Equal("CLASS REVIEW", inmateClassificationNarrative.Narrative);
            //Act
            await _classifyService.UpdateClassificationReview(inmateClassifications);
            //Assert
            inmateClassificationNarrative =
                _fixture.Db.InmateClassificationNarrative.Single(i => i.InmateClassificationNarrativeId == 23);
            Assert.Equal(103, inmateClassificationNarrative.InmateId);
            Assert.Equal("FILE CONVERSION", inmateClassificationNarrative.Narrative);
        }
    }
}
